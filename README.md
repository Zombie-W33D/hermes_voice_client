...
> **What this fork changes:** adds **speak-back voice** — a platform-independent way to *hear* agent responses. Finished reply text is synthesized through the agent's own Hermes profile TTS (`POST /api/message/speak`) into a base64 audio data URL, and the recording is auto-played on the **triggering instance only**. No audio files are persisted (the temp MP3 is deleted server-side once streamed, and stale leftovers are auto-swept), no other device/session ever triggers synthesis, and it's **off by default & per-agent** (use the volume toggle next to the input in that agent's chat). Each agent gets its own voice picked from a per-agent dropdown (`GET/PUT /api/agent/:id/voice`, backed by `hermes config set tts.edge.voice`; `GET /api/voices` lists the ~322 voices the free edge endpoint can synthesize).
