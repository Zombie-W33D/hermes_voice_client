...
> **What this fork changes:** adds **speak-back voice** — a platform-independent way to *hear* agent responses. Finished reply text is synthesized through the agent's own Hermes profile TTS (`POST /api/message/speak`) into a base64 audio data URL, and the recording is auto-played on the **triggering instance only**. No audio files are persisted (the temp MP3 is deleted server-side once streamed, and stale leftovers are auto-swept), no other device/session ever triggers synthesis, and it's **off by default & per-agent** (use the volume toggle next to the input in that agent's chat). Each agent gets its own **voice selected from a per-agent dropdown and remembers the last voice used per agent** (`GET/PUT /api/agent/:id/voice`, backed by `hermes config set tts.edge.voice`; `GET /api/voices` lists the ~322 voices the free edge endpoint can synthesize). **Custom voice cloning:** register any single audio sample as a reusable voice via Coqui XTTS-v2 (one-step per-profile skill `custom-voice-from-sample`); the clone appears in the dropdown under "Custom" and the speak endpoint routes to a warm XTTS daemon automatically.
...
- **Speak-back voice (this fork's addition)** — a platform-independent voice solution for hearing agent responses aloud. Reply text is synthesized through the agent's own Hermes profile TTS (via `POST /api/message/speak`) into a base64 audio data URL, and the recording is autoplayed on the triggering device/instance only. Archive definition: no audio files are persisted — the temp MP3 is deleted server-side as soon as it's streamed to the browser, stale leftovers are auto-swept, and other devices/sessions never trigger synthesis. **Off by default and per-agent**: only the agent you enable the input-bar volume toggle for reads replies aloud, and each agent gets its own **voice** picked from a searchable per-agent dropdown next to the volume toggle that **remembers the last voice used per agent** (persisted to the agent's Hermes profile config). Custom voices cloned from a user-supplied audio sample (Coqui XTTS-v2, one-step skill) show up in the same dropdown and synthesize through a warm XTTS daemon.
